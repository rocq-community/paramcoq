...
let declare_constant ~opaque_access ?(opaque = false) ?(continuation = default_continuation) ~poly ~scope ~kind arity evdr env cst cb name =
...
and declare_module ~opaque_access ?(continuation = ignore) ?name arity mp mb  =
  ...
  | FullStruct, NoFunctor fields ->
     ...
     | (lab, SFBconst cb) ->
       ...
       ignore(declare_constant ~opaque_access ~opaque ~continuation ~poly ~scope ~kind arity evdr env cst cb lab_R)
...
let command_constant ~opaque_access ?(continuation = default_continuation) ~fullname arity constant names =
  ...
  let scope = Locality.(Global ImportDefaultBehavior) in
  let kind = Decls.(IsDefinition Definition) in
  declare_constant ~opaque_access ~continuation ~opaque ~poly ~scope ~kind
    arity (ref evd) env constant cb name
